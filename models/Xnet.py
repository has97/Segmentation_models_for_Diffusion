...
class Encoder2(nn.Module):
    def __init__(self, input_channels,emb_channels=None):
        ...
        self.emb_layers5 = nn.Sequential(
            nn.SiLU(),
            nn.Linear(
                emb_channels,
                64,
        ...
        self.emb_layers6 = nn.Sequential(
            nn.SiLU(),
            nn.Linear(
                emb_channels,
                128,
        ...
        self.emb_layers7 = nn.Sequential(
            nn.SiLU(),
            nn.Linear(
                emb_channels,
                256,
    ...
    def forward(self, x,emb):
        ...
        #encoder 5
        emb_out =  self.emb_layers5(emb).type(x.dtype)
        while len(emb_out.shape) < len(x.shape):
            emb_out = emb_out[..., None]
        x += emb_out
        x = self.enco5(x)
        ...
        #encoder 6
        emb_out =  self.emb_layers6(emb).type(x.dtype)
        while len(emb_out.shape) < len(x.shape):
            emb_out = emb_out[..., None]
        x += emb_out
        x = self.enco6(x)
        ...
        #encoder 7
        emb_out =  self.emb_layers7(emb).type(x.dtype)
        while len(emb_out.shape) < len(x.shape):
            emb_out = emb_out[..., None]
        x += emb_out
        x = self.enco7(x)
...
class XNetBase(nn.Module):
    ...
    def __init__(        
        ...
        self.encoder1 = Encoder1(input_channels=1,emb_channels=time_dim)
        self.encoder2 = Encoder2(input_channels=1,emb_channels=time_dim)
    ...
    def forward(self, x, time, x_self_cond = None):
        ...
        x, id, skip = self.encoder1(x,t1)
        emb = t1
        
        #x = F.max_unpool2d(x, id[2], kernel_size=2, stride=2)
        #decoder 1
        x = self.deco1(x)
        #print("x.shape :",x.shape,"\n skip[1].shape = ",skip[1].shape)
        ...
        while len(emb_out.shape) < len(x.shape):
            emb_out = emb_out[..., None]
        x += emb_out
        #print("x.shape :",x.shape,"x1.sahpe: ",x1.shape)
        x += x1
        x = self.deco7(x)
